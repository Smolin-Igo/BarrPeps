    ...
    try {
        ...
        setTimeout(function() {
            if (selectedSources.length > 0) {
                document.querySelectorAll('#sourceDropdown input').forEach(function(cb) {
            ...
            if (selectedMods.length > 0) {
                document.querySelectorAll('#modDropdown input').forEach(function(cb) {
...
function convertThreeToOne(threeLetter) {
    ...
    return map[threeLetter.toUpperCase()] || '';
}
...
function parseSSBOND(pdbContent) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('SSBOND')) {
            ...
            bonds.push({ chain1: chain1, res1: res1, chain2: chain2, res2: res2 });
        }
    }
    return bonds;
}
...
for (var i = 0; i < 100; i++) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('ATOM') || line.startsWith('HETATM')) {
            ...
            if (resName === 'CYS' && (atomName === 'SG' || atomName === 'S')) {
                ...
                sgAtoms[chainId + '_' + resSeq] = { chain: chainId, resSeq: resSeq, x: x, y: y, z: z };
            }
        }
    }
    ...
    if (peptideInfo && ssbonds.length > 0) {
        ...
        for (var i = 0; i < ssbonds.length; i++) {
            var b = ssbonds[i];
            if (b.chain1 === peptideInfo.chain && b.chain2 === peptideInfo.chain &&
                b.res1 >= peptideInfo.startRes && b.res1 <= peptideInfo.endRes &&
                b.res2 >= peptideInfo.startRes && b.res2 <= peptideInfo.endRes) {
                
                var pairKey = Math.min(b.res1, b.res2) + '_' + Math.max(b.res1, b.res2);
                var relRes1 = b.res1 - offset;
                var relRes2 = b.res2 - offset;
                
                var inDB = false;
                if (dbBonds && dbBonds.length > 0) {
                    for (var d = 0; d < dbBonds.length; d++) {
                        var db1 = parseInt(dbBonds[d].cys1) || 0;
                        var db2 = parseInt(dbBonds[d].cys2) || 0;
                        if ((relRes1 === db1 && relRes2 === db2) || (relRes1 === db2 && relRes2 === db1)) {
                            inDB = true;
                            break;
                        }
                    }
                ...
                if (!used[pairKey]) {
                    used[pairKey] = true;
                    var key1 = b.chain1 + '_' + b.res1;
                    var key2 = b.chain2 + '_' + b.res2;
                    if (sgAtoms[key1] && sgAtoms[key2]) {
                        bonds.push({ atom1: sgAtoms[key1], atom2: sgAtoms[key2] });
                    }
                }
            }
        }
    ...
    if (peptideInfo && peptideInfo.residues.length > 0) {
        ...
        for (var i = 0; i < peptideInfo.residues.length; i++) {
            ...
            pdbViewer.addStyle(
                ...
                { cartoon: { color: clr, opacity: 0.95 } }
            );
        }
    } else {
...
if (pdbHeader.title) {
    ...
    setTimeout(function() {
        ...
        var ec = parentDiv.querySelector('.structure-controls');
        if (ec) ec.remove();
    ...
    if (type === 'cartoon') {
        ...
        if (peptideInfo && peptideInfo.residues.length > 0) {
            ...
            for (var i = 0; i < peptideInfo.residues.length; i++) {
                pdbViewer.addStyle(
                    { chain: peptideInfo.chain, resi: peptideInfo.residues[i].resSeq },
                    { cartoon: { color: getRainbowColor(i, peptideInfo.residues.length), opacity: 0.95 } }
                );
            }
        } else {
            pdbViewer.setStyle({}, { cartoon: { colorscheme: 'ss', opacity: 0.85 } });
...
function switchPDB(index) {
    ...
    var span = document.getElementById('currentPdbId');
    var link = document.getElementById('rcsbLink');
    if (span) span.textContent = s.id;
    if (link) link.href = 'https://www.rcsb.org/structure/' + s.id;
...
function calculateAADistribution() {
    ...
    peptidesData.forEach(function(p) {
        (p.sequence_clean || '').split('').forEach(function(a) {
            if (cnt[a] !== undefined) { cnt[a]++; total++; }
        });
    });
    var r = {};
...
function createClickableLengthChart() {
    ...
    if (!canvas) {
        console.warn('lengthChart canvas not found');
        return;
    }
    ...
    var allLengths = [];
    for (var i = 0; i < peptidesData.length; i++) {
        var len = parseInt(peptidesData[i].length) || 0;
        if (len > 0) allLengths.push(len);
    }
    
    if (allLengths.length === 0) {
        console.warn('No length data');
        return;
    }
    
    // Создаем бины по 5
    var maxLen = Math.max.apply(null, allLengths);
    var binSize = 5;
    var bins = {};
    
    for (var start = 1; start <= maxLen; start += binSize) {
        var end = start + binSize - 1;
        bins[start + '-' + end] = 0;
    }
    
    for (var i = 0; i < allLengths.length; i++) {
        var l = allLengths[i];
        var binIndex = Math.floor((l - 1) / binSize);
        var start = binIndex * binSize + 1;
        var end = start + binSize - 1;
        var key = start + '-' + end;
    ...
    for (var key in bins) {
        if (bins[key] > 0) {
            cleanBins[key] = bins[key];
        }
    }
    
    var labels = Object.keys(cleanBins);
    var values = Object.values(cleanBins);
    ...
    window._lengthChartData = {
        labels: labels,
        values: values
    };
    
    // Уничтожаем старый график
    if (window.lengthChartInstance && typeof window.lengthChartInstance.destroy === 'function') {
        window.lengthChartInstance.destroy();
    }
    
    window.lengthChartInstance = new Chart(canvas, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
        ...
        options: {
            ...
            onClick: function(e, elements) {
                ...
                var idx = elements[0].dataIndex;
                var label = window._lengthChartData.labels[idx];
                if (!label) return;
                ...
                console.log('📊 Length filter:', minVal + '-' + maxVal);
    ...
    console.log('✅ Length chart created with', labels.length, 'bins');
}
...
function createClickableAAChart() {
    var canvas = document.getElementById('aaChart');
    if (!canvas) {
        console.warn('aaChart canvas not found');
        return;
    }
    if (typeof Chart === 'undefined') {
        console.warn('Chart.js not loaded');
    ...
    if (window.aaChartInstance && typeof window.aaChartInstance.destroy === 'function') {
        window.aaChartInstance.destroy();
    }
    
    window.aaChartInstance = new Chart(canvas, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
        ...
        options: {
            ...
            onClick: function(e, elements) {
                ...
                var idx = elements[0].dataIndex;
                var aa = window._aaChartData.labels[idx];
                if (!aa) return;
                
                console.log('🔬 AA filter:', aa);
    ...
    canvas.style.cursor = 'pointer';
    console.log('✅ AA chart created with', labels.length, 'amino acids');
}
...
function initHomePage() {
    ...
    // Ждем загрузки данных и Chart.js
    var checkReady = function(attempts) {
        ...
        if (peptidesData && peptidesData.length > 0 && typeof Chart !== 'undefined') {
            ...
            console.log('⏳ Waiting for data... (attempt ' + (attempts + 1) + ')');
            setTimeout(function() {
...
function updateHomeStats() {
    ...
    var ae = document.getElementById('totalPeptides');
    var le = document.getElementById('avgLength');
...
function initModificationSelector() {
    ...
    peptidesData.forEach(function(p) {
        (p.modifications || []).forEach(function(m) {
            if (m && m !== 'N/A' && m !== '') mt[m.replace(/_/g, ' ')] = m;
        });
    });
    var sk = Object.keys(mt).sort();
    dd.innerHTML = sk.map(function(k) {
        return '<div class="multiselect-option"><input type="checkbox" value="' + mt[k] + '" onchange="updateModSelectionAndFilter()"><label>' + k + '</label></div>';
    }).join('');
}
...
function initSourceSelector() {
    ...
    dd.innerHTML = sk.map(function(k) {
        return '<div class="multiselect-option"><input type="checkbox" value="' + src[k] + '" onchange="updateSourceSelectionAndFilter()"><label>' + k + '</label></div>';
    }).join('');
}
...
function downloadFASTA() {
    ...
    var b = new Blob([fa], { type: 'text/plain' });
...
function downloadFullCSV() {
    ...
    var rows = filteredPeptides.map(function(p) {
        return [p.id, p.peptide_name, p.sequence_one_letter, p.sequence_clean, p.length, p.molecular_weight, p.molecular_formula, p.structure_type, p.disulfide_bridge, p.source_organism, (p.modifications || []).join('; '), (p.pdb_ids || []).join('; '), p.has_pdb ? 'Yes' : 'No'];
    });
...
function displayPeptideDetail(peptide, pdbContents, pdbIds) {
    ...
    var disulfideH = '';
    if (peptide.disulfide_bridge && peptide.disulfide_bridge.toLowerCase() !== 'no' && peptide.disulfide_bridge !== '') {
        disulfideH = '<div class="detail-section"><h3>Disulfide Bonds</h3><div class="detail-row"><span class="detail-value" style="color:#d69e2e;font-weight:600;">' + peptide.disulfide_bridge + '</span></div></div>';
    }
